...
object MyModule {
  ...
  def fib(n: Int): Int = {
    @annotation.tailrec
    def iter(a: Int, b: Int, i: Int): Int =
      if (i == 0) b
      else iter(a + b, a, i - 1)
...
object PolymorphicFunctions {
  ...
  // TEST in console:
  // PolymorphicFunctions.isSorted(Array(1, 2, 3),
  //   (a: Int, b: Int) => (a <= b))
  def isSorted[A](as: Array[A], gt: (A, A) => Boolean): Boolean = {
    @annotation.tailrec
    def loop(i: Int, el: A): Boolean =
      if (i == as.length) true
      else if (gt(el, as(i))) loop(i + 1, as(i))
      else false
  ...
  // Test: PolymorphicFunctions.partial1(5, (a: Int, b: Int) => a + b)(5)
